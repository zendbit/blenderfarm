...
class NodeMonitor(Thread):
    ...
    def _sort_node_by_cpu_load(self):
        ...
        if self._s_node_cpuinfo != None:
            ...
            data_node = self._node_sort_cpuinfo.findall(Constants.C_STR_NODE)
            
            # save sorted data node
            data_node_sort = []
            
            for elem in data_node:
                data_node_sort.append((float(elem.attrib[Constants.C_STR_LOAD_FACTOR]),
                                        float(elem.attrib[Constants.C_STR_CPU_USAGE_AVR]),
                                        elem))
    ...
    def update_node_cpuinfo(ip_addr, node_data):
        ...
        if NodeMonitor._s_node_cpuinfo == None:
            if os.path.isfile(Constants.C_STR_NODE_CPUINFO_FILE):
                NodeMonitor._s_node_cpuinfo = ET.parse(Constants.C_STR_NODE_CPUINFO_FILE).getroot()
            else:
                NodeMonitor._s_node_cpuinfo = ET.Element(Constants.C_STR_NODES)
                
        else:
            # find and ip address in node info file
            # if not exist then register it
            # find all node inside nodes tag
            node_cpuinfo_item = NodeMonitor._s_node_cpuinfo.find('.//'+Constants.C_STR_NODE+'/[@'+Constants.C_STR_IP+'=\''+ip_addr+'\']')
        
            # register new node if node not exist
            if node_cpuinfo_item == None:
                node_cpuinfo_item = ET.SubElement(NodeMonitor._s_node_cpuinfo, Constants.C_STR_NODE)
            ...
            node_cpuinfo_item.set(Constants.C_STR_LOAD_FACTOR, str(node_data[Constants.C_STR_LOAD_FACTOR]))
            ...
            node_cpuinfo_item.set(Constants.C_STR_RUNNING_THREAD, str(node_data[Constants.C_STR_RUNNING_THREAD]))
    ...
    def __serialize_data(self):
        ...
        if NodeMonitor._s_node_cpuinfo != None:
            ...
            ET.ElementTree(NodeMonitor._s_node_cpuinfo).write(Constants.C_STR_NODE_CPUINFO_FILE, encoding='UTF-8')
    ...
    def __init_node_server(self, config):
        if not self._thread_node_listener.is_alive():
            ...
            self._server_monitor = socketserver.TCPServer((config[Constants.C_STR_IP], config[Constants.C_STR_PORT]), NodeHandler)
    ...
    def __check_node_info_file(self):
        if not os.path.isfile(Constants.C_STR_NODE_CPUINFO_FILE):
            
            # create xml parent structure
            node_info = ET.Element(Constants.C_STR_NODES)
            ET.ElementTree(node_info).write(Constants.C_STR_NODE_CPUINFO_FILE, encoding='UTF-8')
    ...
    def set_server_status(self, status):
        if not os.path.isfile(Constants.C_STR_SERVER_INFO_FILE):
            
            # create file server info file
            server_info = ET.Element(Constants.C_STR_SERVER)
            server_info.set(Constants.C_STR_STATUS, str(status))
            
            ET.ElementTree(server_info).write(Constants.C_STR_SERVER_INFO_FILE, encoding='UTF-8')
            
        else:
            server_info = ET.parse(Constants.C_STR_SERVER_INFO_FILE).getroot()
    ...
    def get_server_status(self):
        server_info = ET.parse(Constants.C_STR_SERVER_INFO_FILE).getroot()
        return int(server_info.attrib[Constants.C_STR_STATUS])
    ...
    def stop_service(self):
        ...
        # delete server info
        os.remove(Constants.C_STR_SERVER_INFO_FILE)
